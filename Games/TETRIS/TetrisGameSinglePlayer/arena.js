...
class Arena {
    ...
    */
    constructor(w, h) {
        const matrix = []
        // atenção, enquanto h não for zero (h--) e vai decrementando
        while (h--) {
    ...
    collide(player) {
        ...
        for (let y = 0; y < m.length; ++y) {
            for (let x = 0; x < m[y].length; ++x) {
                if (m[y][x] !== 0 && // player  is not 0
                    (this.matrix[y + o.y] && //desta arena row exists
                        this.matrix[y + o.y][x + o.x]) !== 0) { // desta arena not 0
                    return true
                }
            }
        }
    ...
    */
    sweep() {
        ...
        outer: for (let y = this.matrix.length - 1; y > 0; --y) {
            ...
            tetris.player.score += rowCount * 10
            rowCount *= 2
        }
    }
}